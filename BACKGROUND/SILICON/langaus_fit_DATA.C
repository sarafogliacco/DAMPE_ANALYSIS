...
Double_t langaufun(Double_t *x, Double_t *par) {
   ...
   //   par[3]=Sigma della Gaussiana
      ...
      Double_t np = 500.0;      // numero di step di convoluzione 100
      Double_t sc =   7.0;      // convoluzione estesa a +-sc sigma della Gaussiana 5
...
Int_t langaupro(Double_t *params, Double_t &maxx, Double_t &FWHM) {
   ...
   void langaus_fit_DATA(){

   TFile *f = new TFile("/mnt/c/Users/saraf/Desktop/DAMPE_ANALYSIS/BACKGROUND/out_root/Skim_120months_SATcorr_STKcut1200_PSDcutXY_v4.root");
   ...
   TH1F *h08 = (TH1F*)f->Get("h08");
	...
	int j;
	cout << " valore di j (=0 inizializzazione, =1-8 fit histo)" << endl;
	cin >> j ;
   ...
   if(j==1) {fr[0]=14.55;   fr[1]=15.15;}
   if(j==2) {fr[0]=14.6;   fr[1]=15.2;}
   if(j==3) {fr[0]=14.6;   fr[1]=15.3;}
   if(j==4) {fr[0]=14.6;   fr[1]=15.4;}
   if(j==5) {fr[0]=14.65;   fr[1]=15.5;}
   if(j==6) {fr[0]=14.65;   fr[1]=15.5;}
   if(j==7) {fr[0]=14.65;   fr[1]=15.7;}
   if(j==8) {fr[0]=14.65;   fr[1]=15.75;}
   ...
   xxx->Fit("landau","","",fr[0],fr[1]);
	...
	plhi[2] = 0.3*fit->GetParameter(0); //
	plhi[3] = 0.8*fit->GetParameter(2);   // GSIGMA (sigma della gaussiana) = 0.9 sigma della landau
   ...
   Double_t peak = xxx->GetBinCenter(xxx->GetMaximumBin());
   Double_t rms  = xxx->GetRMS();

   sv[1] = peak;          // MPV più robusto
   sv[0] = 0.2 * rms;     // width stimata
   sv[2] = xxx->Integral();   
   sv[3] = 0.3 * rms;

   pllo[0] = 0.01;   plhi[0] = 0.08;     // Width
   pllo[1] = peak - 1.0; plhi[1] = peak + 1.0;
   pllo[2] = 0.01 * sv[2]; plhi[2] = 10 * sv[2];
   pllo[3] = 0.05;  plhi[3] = 1.5;      // GSigma più largo
   */
	/////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   xxx->GetYaxis()->SetTitleOffset(1.3); xxx->GetXaxis()->SetRangeUser(14.,17.0);
  	TH1F *hxx = (TH1F*)(xxx->Clone());
   /*
   	fitsnr->Draw("same");
  	TF1* langaufun = (TF1*)(xxx->FindObject("langaufun"));
  	fitsnr->SetLineColor(kMagenta);
    hxx->SetMarkerStyle(20); hxx->SetMarkerSize(0.6);
    hxx->SetMarkerColor(39); hxx->SetLineColor(39);
  	hxx->Draw("same");
  	c1->Update();
   */
   xxx->Draw();
...
/**/
   ofstream fout;
   fout.open("/mnt/c/Users/saraf/Desktop/DAMPE_ANALYSIS/BACKGROUND/langaus_results/langaus_fit_DATA_SILICON_STKcut1200_PSDcutXY.txt",ios::app);
