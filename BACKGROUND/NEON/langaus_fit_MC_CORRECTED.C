...
void langaus_fit_MC_SULFUR_CORRECTED(){
   ...
   TFile *f = new TFile("../out_root/MC_FTFP_SULFUR_SATcorr_STKcut1200_PSDcutXY_CORRECTED_100GeV-500TeV_v4.root");
   ...
   if(j==1) {fr[0]=15.55; fr[1]=17.1;}
   if(j==2) {fr[0]=15.5; fr[1]=17.1;}
   if(j==3) {fr[0]=15.5; fr[1]=17.2;}
   if(j==4) {fr[0]=15.5; fr[1]=17.2;}
   if(j==5) {fr[0]=15.5; fr[1]=17.2;}
   if(j==6) {fr[0]=15.5; fr[1]=17.2;}
   if(j==7) {fr[0]=15.5; fr[1]=17.3;}
   if(j==8) {fr[0]=15.5; fr[1]=17.3;}
   if(j==9) {fr[0]=15.6; fr[1]=17.4;}
   if(j==10){fr[0]=15.6; fr[1]=18.0;}
   ...
   if(j==10) xxx = (TH1F*)h10->Clone();
	...
	plhi[3] = 1.4*fit->GetParameter(2);   // GSIGMA (sigma della gaussiana) = 0.9 sigma della landau
...
/**/
   ofstream fout;
   fout.open("langaus_results/langaus_fit_MC_SULFUR_STKcut1200_PSDcutXY_CORRECTED.txt",ios::app);
