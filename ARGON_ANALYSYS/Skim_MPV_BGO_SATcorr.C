{
	...
	TH1F *h01=new TH1F("h01", "100 GeV < E_{BGO} < 178 GeV",350,15,22); h01->GetXaxis()->SetTitle("PSD charge"); h01->GetYaxis()->SetTitle("events"); h01->SetLineColor(kBlack); h01->SetMarkerColor(kBlack); h01->Sumw2();
	TH1F *h02=new TH1F("h02", "178 GeV < E_{BGO} < 316 GeV",350,15,22); h02->GetXaxis()->SetTitle("PSD charge"); h02->GetYaxis()->SetTitle("events"); h02->SetLineColor(kBlack); h02->SetMarkerColor(kBlack); h02->Sumw2();
	TH1F *h03=new TH1F("h03", "316 GeV < E_{BGO} < 562 GeV",350,15,22); h03->GetXaxis()->SetTitle("PSD charge"); h03->GetYaxis()->SetTitle("events"); h03->SetLineColor(kBlack); h03->SetMarkerColor(kBlack); h03->Sumw2();
	TH1F *h04=new TH1F("h04", "562 GeV < E_{BGO} < 1.0 TeV",350,15,22); h04->GetXaxis()->SetTitle("PSD charge"); h04->GetYaxis()->SetTitle("events"); h04->SetLineColor(kBlack); h04->SetMarkerColor(kBlack); h04->Sumw2();
	TH1F *h05=new TH1F("h05", "1.0 TeV < E_{BGO} < 1.8 TeV", 200,15,22); h05->GetXaxis()->SetTitle("PSD charge"); h05->GetYaxis()->SetTitle("events"); h05->SetLineColor(kBlack); h05->SetMarkerColor(kBlack); h05->Sumw2();
	TH1F *h06=new TH1F("h06", "1.8 TeV < E_{BGO} < 3.2 TeV", 200,15,22); h06->GetXaxis()->SetTitle("PSD charge"); h06->GetYaxis()->SetTitle("events"); h06->SetLineColor(kBlack); h06->SetMarkerColor(kBlack); h06->Sumw2();
	TH1F *h07=new TH1F("h07", "3.2 TeV < E_{BGO} < 10.0 TeV",200,15,22); h07->GetXaxis()->SetTitle("PSD charge"); h07->GetYaxis()->SetTitle("events"); h07->SetLineColor(kBlack); h07->SetMarkerColor(kBlack); h07->Sumw2();
    TH1F *h08=new TH1F("h08","10.0 TeV < E_{BGO} < 100.0 TeV",80,15,22);h08->GetXaxis()->SetTitle("PSD charge"); h08->GetYaxis()->SetTitle("events"); h08->SetLineColor(kBlack); h08->SetMarkerColor(kBlack); h08->Sumw2();
	...
	// -------------------------------------------------------------- 	DATA SAVING...
	TFile *f = TFile::Open("out_root/Skim_120months_SATcorr_STKcut1200_PSDcutXY_v3.root", "RECREATE");
	f->cd();
