...
void cfr_DATA_MC_argon_mpv(){
    ...
    sprintf(tit, "; BGO energy (GeV); ARAGON MPV ");
    f1->SetTitle(tit); 
    ...
    ifstream infileMC("langaus_results/langaus_results_argon_MC_corrected.txt");
    ...
    ///////////////////////////////////// FLIGHT DATA
    ifstream infileDATA("langaus_results/langaus_results_argon_DATA.txt");
    ...
    cout << "TString MPV_DATA = \"(" << fitDATA->GetParameter(0) << "+(" << fitDATA->GetParameter(1) << "*log10(BGO_EnergyG_QuenchSatCorr_ML_ions))" << "+(" << fitDATA->GetParameter(2) << "*pow(log10(BGO_EnergyG_QuenchSatCorr_ML_ions),2))" << "+(" << fitDATA->GetParameter(3) << "*pow(log10(BGO_EnergyG_QuenchSatCorr_ML_ions),3))" << "+(" << fitDATA->GetParameter(4) << "*pow(log10(BGO_EnergyG_QuenchSatCorr_ML_ions),4))" << ")\";" << endl;
    cout << endl;

    auto legend = new TLegend(0.2,0.67,0.4,0.86);
    legend->AddEntry(gmpv_data," Ar DATA","lep");
    legend->AddEntry(gmpv_mc," Ar MC","lep");
    legend->SetLineColor(0);
    legend->Draw();
    
    //c1->SaveAs("plots/cfr_DATA_MC_argon_mpv.png");
    //c1->SaveAs("plots/cfr_DATA_MC_argon_mpv_CORRECTED.png");
