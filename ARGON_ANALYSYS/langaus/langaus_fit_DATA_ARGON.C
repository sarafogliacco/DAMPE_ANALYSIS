...
Int_t langaupro(Double_t *params, Double_t &maxx, Double_t &FWHM) {
   ...
   void langaus_fit_DATA_ARGON(){

   TFile *f = new TFile("../out_root/Skim_120months_SATcorr_STKcut1200_PSDcutXY_v3.root");
   ...
   if(j==1) {fr[0]=17.8; fr[1]=18.65;}
   if(j==2) {fr[0]=17.85; fr[1]=18.7;}
   if(j==3) {fr[0]=17.8; fr[1]=18.8;}
   if(j==4) {fr[0]=17.8; fr[1]=18.8;}
   if(j==5) {fr[0]=17.8; fr[1]=18.8;}
   if(j==6) {fr[0]=17.9; fr[1]=18.9;}
   if(j==7) {fr[0]=18.0; fr[1]=19.0;}
   if(j==8) {fr[0]=17.8; fr[1]=19.15;}
   ...
   xxx->Fit("landau","","",fr[0],fr[1]);
	...
	plhi[2] = 0.4*fit->GetParameter(0); // 0.35 up to h04, 0.4 da h05 a h08
	plhi[3] = 1.0*fit->GetParameter(2);   // GSIGMA (sigma della gaussiana) = 0.9 sigma della landau
   ...
   */
   ...
   xxx->GetYaxis()->SetTitleOffset(1.3); xxx->GetXaxis()->SetRangeUser(17.4,19.2);
...
/**/
   ofstream fout;
   fout.open("langaus_results/langaus_fit_DATA_ARGON_STKcut1200_PSDcutXY.txt",ios::app);
