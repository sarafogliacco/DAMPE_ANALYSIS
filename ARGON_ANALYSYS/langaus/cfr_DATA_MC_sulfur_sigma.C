...
void cfr_DATA_MC_sulfur_sigma(){
    ...
    ifstream infileMC("PSDX/results_langaus_fit_DATA_PSDX.txt");
    //ifstream infileMC("PSDY/results_langaus_fit_DATA_PSDY.txt");
    ...
    cout << "TString sigma_DATAX = \"(" << fitMC->GetParameter(0) << "+(" << fitMC->GetParameter(1) << "*log10(BGO_EnergyG_QuenchSatCorr_ML_ions))" << "+(" << fitMC->GetParameter(2) << "*pow(log10(BGO_EnergyG_QuenchSatCorr_ML_ions),2))" << "+(" << fitMC->GetParameter(3) << "*pow(log10(BGO_EnergyG_QuenchSatCorr_ML_ions),3))" << ")\";" << endl;
    cout << endl;
    ...
    cout << "TString sigma_DATAY = \"(" << fitDATA->GetParameter(0) << "+(" << fitDATA->GetParameter(1) << "*log10(BGO_EnergyG_SatCorr_ML_ions))" << "+(" << fitDATA->GetParameter(2) << "*pow(log10(BGO_EnergyG_SatCorr_ML_ions),2))" << "+(" << fitDATA->GetParameter(3) << "*pow(log10(BGO_EnergyG_SatCorr_ML_ions),3))" << ")\";" << endl;
    cout << endl;

    auto legend = new TLegend(0.2,0.67,0.4,0.86);
    legend->AddEntry(gwidth_data," S DATA","lep");
    legend->AddEntry(gwidth_mc," S MC","lep");
