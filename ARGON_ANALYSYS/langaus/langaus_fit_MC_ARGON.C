...
void langaus_fit_MC_ARGON(){

   TFile *f = new TFile("../out_root/MC_FTFP_ARGON_SATcorr_STKcut1200_PSDcutXY.root");
   ...
   if(j==1) {fr[0]=18.7; fr[1]=19.5;}
   if(j==2) {fr[0]=18.8; fr[1]=19.6;}
   if(j==3) {fr[0]=18.85; fr[1]=19.7;}
   if(j==4) {fr[0]=18.87; fr[1]=19.8;}
   if(j==5) {fr[0]=18.9; fr[1]=19.8;}
   if(j==6) {fr[0]=18.95; fr[1]=19.85;}
   if(j==7) {fr[0]=18.95; fr[1]=19.9;}
   if(j==8) {fr[0]=18.95; fr[1]=19.95;}
   if(j==9) {fr[0]=18.95; fr[1]=20.2;}
   if(j==10){fr[0]=19.1; fr[1]=20.7;}
   ...
   */
   ...
   printf("Fitting done\nPlotting results...\n");
    ...
    xxx->GetYaxis()->SetTitleOffset(1.3); xxx->GetXaxis()->SetRangeUser(18.0,21.2);
...
/**/
   ofstream fout;
   fout.open("langaus_results/langaus_fit_MC_ARGON_STKcut1200_PSDcutXY.txt",ios::app);
