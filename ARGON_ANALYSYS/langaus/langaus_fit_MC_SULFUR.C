...
void langaus_fit_MC_SULFUR(){

   TFile *f = new TFile("../out_root/MC_FTFP_SULFUR_SATcorr_STKcut1200_PSDcutXY_100GeV-500TeV_v2.root");
   ...
   if(j==1) {fr[0]=16.8; fr[1]=17.20;}
   if(j==2) {fr[0]=16.8; fr[1]=17.25;}
   if(j==3) {fr[0]=16.85; fr[1]=17.3;}
   if(j==4) {fr[0]=16.85; fr[1]=17.35;}
   if(j==5) {fr[0]=16.9; fr[1]=17.35;}
   if(j==6) {fr[0]=16.9; fr[1]=17.4;}
   if(j==7) {fr[0]=16.9; fr[1]=17.55;}
   if(j==8) {fr[0]=16.95; fr[1]=17.6;}
   if(j==9) {fr[0]=17.0; fr[1]=17.85;}
   if(j==10){fr[0]=17.1; fr[1]=18.2;}
   ...
   if(j==10) xxx = (TH1F*)h10->Clone();
	...
	plhi[3] = 0.9*fit->GetParameter(2);   // GSIGMA (sigma della gaussiana) = 0.9 sigma della landau
...
/**/
   ofstream fout;
   fout.open("langaus_results/langaus_fit_MC_SULFUR_STKcut1200_PSDcutXY.txt",ios::app);
