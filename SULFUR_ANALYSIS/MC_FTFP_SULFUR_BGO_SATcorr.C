{
	...
	TH1F *h05=new TH1F("h05", "1.0 TeV < E_{BGO} < 1.8 TeV", 600,10.,20.); h05->GetXaxis()->SetTitle("PSD charge"); h05->GetYaxis()->SetTitle("normalized MC events"); h05->SetLineColor(kViolet); h05->SetMarkerColor(kViolet); h05->Sumw2();
	TH1F *h06=new TH1F("h06", "1.8 TeV < E_{BGO} < 3.2 TeV", 600,10.,20.); h06->GetXaxis()->SetTitle("PSD charge"); h06->GetYaxis()->SetTitle("normalized MC events"); h06->SetLineColor(kViolet); h06->SetMarkerColor(kViolet); h06->Sumw2();
	TH1F *h07=new TH1F("h07", "3.2 TeV < E_{BGO} < 5.6 TeV", 600,10.,20.); h07->GetXaxis()->SetTitle("PSD charge"); h07->GetYaxis()->SetTitle("normalized MC events"); h07->SetLineColor(kViolet); h07->SetMarkerColor(kViolet); h07->Sumw2();
	TH1F *h08=new TH1F("h08", "5.6 TeV < E_{BGO} < 10.0 TeV",600,10.,20.); h08->GetXaxis()->SetTitle("PSD charge"); h08->GetYaxis()->SetTitle("normalized MC events"); h08->SetLineColor(kViolet); h08->SetMarkerColor(kViolet); h08->Sumw2();
	TH1F *h09=new TH1F("h09", "10.0 TeV < E_{BGO} < 31.6 TeV",600,10.,20.); h09->GetXaxis()->SetTitle("PSD charge"); h09->GetYaxis()->SetTitle("normalized MC events"); h09->SetLineColor(kViolet); h09->SetMarkerColor(kViolet); h09->Sumw2();
	TH1F *h10=new TH1F("h10", "31.6 TeV < E_{BGO} < 100.0 TeV",600,10.,20.); h10->GetXaxis()->SetTitle("PSD charge"); h10->GetYaxis()->SetTitle("normalized MC events"); h10->SetLineColor(kViolet); h10->SetMarkerColor(kViolet); h10->Sumw2();
	...
	// -------------------------------------------------------------- 	SAVE ON A FILE...
	TFile *f = TFile::Open("out_root/MC_FTFP_SULFUR_SATcorr_STKcut1200_PSDcutXY_100GeV-500TeV_v2.root", "RECREATE");
	f->cd();
