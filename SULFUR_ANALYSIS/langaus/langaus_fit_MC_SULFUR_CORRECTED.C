...
void langaus_fit_MC_SULFUR_CORRECTED(){
   ...
   TFile *f = new TFile("../out_root/MC_FTFP_SULFUR_SATcorr_STKcut1200_PSDcutXY_CORRECTED_100GeV-500TeV_v2.root");
   ...
   if(j==1) {fr[0]=15.7; fr[1]=16.8;}
   if(j==2) {fr[0]=15.8; fr[1]=16.8;}
   if(j==3) {fr[0]=15.75; fr[1]=16.85;}
   if(j==4) {fr[0]=15.8; fr[1]=16.85;}
   if(j==5) {fr[0]=15.8; fr[1]=16.9;}
   if(j==6) {fr[0]=15.8; fr[1]=17.0;}
   if(j==7) {fr[0]=15.8; fr[1]=17.2;}
   if(j==8) {fr[0]=15.9; fr[1]=17.3;}
   if(j==9) {fr[0]=15.9; fr[1]=17.5;}
