...
/*
     std::vector<int> binsPerDecade = {6, 6, 6, 6};
...
*/
        ...
        // 08/06/2026
        TString MPV_DATA =  "(13.0366+(1.01399*log10(BGO_EnergyG_SatCorr_ML_ions))+(-0.321937*pow(log10(BGO_EnergyG_SatCorr_ML_ions),2))+(0.036754*pow(log10(BGO_EnergyG_SatCorr_ML_ions),3)))";
        
        TString sigma_DATA = "(0.205642+(0.00990178*log10(BGO_EnergyG_SatCorr_ML_ions)))";
        ...
        TFile *foutp = new TFile("Out_DATA_120months_SILICON_6bin.root","RECREATE");
        ...
        for (int i = 1; i <= h1Nobs->GetNbinsX(); i++) {
          double entries = h1Nobs->GetBinContent(i);
          std::cout << "Bin " << i
                    << " : " << entries << std::endl;
        }
        /*
