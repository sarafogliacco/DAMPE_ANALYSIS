...
Double_t langaufun(Double_t *x, Double_t *par) {
   ...
   //   par[3]=Sigma della Gaussiana
      ...
      Double_t np = 100.0;      // numero di step di convoluzione 100
      Double_t sc =   5.0;      // convoluzione estesa a +-sc sigma della Gaussiana 5
...
Int_t langaupro(Double_t *params, Double_t &maxx, Double_t &FWHM) {
   ...
   void langaus_fit_DATA_SULFUR(){

   TFile *f = new TFile("PSDX/Skim_120months_SATcorr_STKcut1200_PSDX.root");
   //TFile *f = new TFile("PSDY/Skim_120months_SATcorr_STKcut1200_PSDY.root");
   ...
   if(j==1) {fr[0]=15.7; fr[1]=16.6;}
   if(j==2) {fr[0]=15.7; fr[1]=16.7;}
   if(j==3) {fr[0]=15.7; fr[1]=16.7;}
   if(j==4) {fr[0]=15.70; fr[1]=17.0;} 
   if(j==5) {fr[0]=15.6; fr[1]=17.0;}
   if(j==6) {fr[0]=15.70; fr[1]=17.0;}
   if(j==7) {fr[0]=15.70; fr[1]=16.9;}
   if(j==8) {fr[0]=15.8; fr[1]=17.2;}
   ...
   */
   ...
   xxx->GetYaxis()->SetTitleOffset(1.3); xxx->GetXaxis()->SetRangeUser(15.4,18.);
...
/**/
   ofstream fout;
   fout.open("PSDX/langaus_fit_DATA_SULFUR_STKcut1200_PSDX.txt",ios::app);
   //fout.open("PSDY/langaus_fit_DATA_SULFUR_STKcut1200_PSDY.txt",ios::app);
