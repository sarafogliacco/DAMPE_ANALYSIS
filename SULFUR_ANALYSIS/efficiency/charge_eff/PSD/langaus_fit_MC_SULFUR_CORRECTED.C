...
Double_t langaufun(Double_t *x, Double_t *par) {
   ...
   //   par[3]=Sigma della Gaussiana
      ...
      Double_t np = 700.0;      // numero di step di convoluzione
      Double_t sc =   7.0;      // convoluzione estesa a +-sc sigma della Gaussiana
...
void langaus_fit_MC_SULFUR_CORRECTED(){

   TFile *f = new TFile("PSDX/MC_FTFP_SULFUR_SATcorr_STKcut1200_PSDcutXY_CORRECTED_PSDX.root");
   //TFile *f = new TFile("PSDY/MC_FTFP_SULFUR_SATcorr_STKcut1200_PSDcutXY_CORRECTED_PSDY.root");
   ...
   TH1F *h10 = (TH1F*)f->Get("h10");
	...
	TCanvas *c1=new TCanvas("c1","gaussian",800,800); //c1->SetLogy(); c1->Divide(4,3);
   ...
   if(j==1) {fr[0]=15.5; fr[1]=17.0;}
   if(j==2) {fr[0]=15.5; fr[1]=17.3;}
   if(j==3) {fr[0]=15.5; fr[1]=17.3;}
   if(j==4) {fr[0]=15.5; fr[1]=17.3;}
   if(j==5) {fr[0]=15.5; fr[1]=17.4;}
   if(j==6) {fr[0]=15.5; fr[1]=17.4;}
   if(j==7) {fr[0]=15.5; fr[1]=17.4;}
   if(j==8) {fr[0]=15.5; fr[1]=17.4;}
   if(j==9) {fr[0]=15.5; fr[1]=17.4;}
   if(j==10){fr[0]=16.0; fr[1]=18.0;}
   ...
   for (int i = 0; i < 4; i++) {
      if (sv[i] < pllo[i]) sv[i] = pllo[i] + 0.1*(plhi[i]-pllo[i]);
      if (sv[i] > plhi[i]) sv[i] = plhi[i] - 0.1*(plhi[i]-pllo[i]);
   }
      

   pllo[0] = 0.01;  plhi[0] = 0.2;   // Width
   pllo[1] = sv[1] - 1.0;  plhi[1] = sv[1] + 1.0; // MPV
   pllo[2] = 0.1 * sv[2];  plhi[2] = 10.0 * sv[2]; // Area
   pllo[3] = 0.01;   plhi[3] = 1.5;    // GSigma
   */
...
/**/
   ofstream fout;
   fout.open("PSDX/langaus_fit_MC_SULFUR_STKcut1200_PSDX_CORRECTED.txt",ios::app);
   //fout.open("PSDY/langaus_fit_MC_SULFUR_STKcut1200_PSDY_CORRECTED.txt",ios::app);
