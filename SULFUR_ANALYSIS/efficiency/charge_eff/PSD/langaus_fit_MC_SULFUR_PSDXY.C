...
void langaus_fit_MC_SULFUR_PSDXY(){

   TFile *f = new TFile("PSDX/MC_FTFP_SULFUR_PSDXpathweighted_SATcorr_STKcut1200_PSDX.root");
   //TFile *f = new TFile("PSDY/MC_FTFP_SULFUR_PSDYpathweighted_SATcorr_STKcut1200_PSDY.root");
   ...
   if(j==1) {fr[0]=16.6; fr[1]=17.5;}
   if(j==2) {fr[0]=16.7; fr[1]=17.5;}
   if(j==3) {fr[0]=16.7; fr[1]=17.6;}
   if(j==4) {fr[0]=16.8; fr[1]=17.6;}
   if(j==5) {fr[0]=16.8; fr[1]=17.8;}
   if(j==6) {fr[0]=16.8; fr[1]=17.8;}
   if(j==7) {fr[0]=16.8; fr[1]=17.8;}
   if(j==8) {fr[0]=16.8; fr[1]=17.9;}
   if(j==9) {fr[0]=16.9; fr[1]=18.0;}
   if(j==10){fr[0]=17.0; fr[1]=18.5;}
   ...
   printf("Fitting done\nPlotting results...\n");
    ...
    xxx->GetYaxis()->SetTitleOffset(1.3); xxx->GetXaxis()->SetRangeUser(15.8,19.2);
...
/**/
   ofstream fout;
   fout.open("PSDX/langaus_fit_MC_SULFUR_STKcut1200_PSDX.txt",ios::app);
   //fout.open("PSDY/langaus_fit_MC_SULFUR_STKcut1200_PSDY.txt",ios::app);
