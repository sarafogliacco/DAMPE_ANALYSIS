...
void cfr_DATA_MC_sulfur_sigma(){
    ...
    ifstream infileDATA("PSDX/results_langaus_fit_DATA_PSDX.txt");
    //ifstream infileDATA("PSDY/results_langaus_fit_DATA_PSDY.txt");
    ...
    TF1 *fitDATA = new TF1("fitDATA","[0]+[1]*log10(x)+[2]*log10(x)**2",100.,100000.);
    //TF1 *fitMC = new TF1("fitMC","[0]+[1]*log10(x)",100.,100000.);
    fitDATA->SetLineColor(kViolet);

    ///////////////////////////////////// FLIGHT DATA
    int ii=0; 
    //ifstream infileMC("PSDX/results_langaus_fit_MC_PSDX.txt");
    //ifstream infileMC("PSDY/results_langaus_fit_MC_PSDY.txt");

    ifstream infileMC("PSDX/results_langaus_fit_MC_CORRECTED_PSDX.txt");
    //ifstream infileMC("PSDY/results_langaus_MC_CORRECTED_PSDY.txt");
    ...
    TF1 *fitMC = new TF1("fitMC","[0]+[1]*log10(x)+[2]*log10(x)**2",100.,100000.);
    //TF1 *fitDATA = new TF1("fitDATA","[0]+[1]*log10(x)",100.,100000.);
    fitMC->SetLineColor(39);
    ...
    gwidth_mc->Fit("fitDATA","r");
    cout << "TString sigma_DATA = \"(" << fitDATA->GetParameter(0) << "+(" << fitDATA->GetParameter(1) << "*log10(BGO_EnergyG_QuenchSatCorr_ML_ions))" << "+(" << fitDATA->GetParameter(2) << "*pow(log10(BGO_EnergyG_QuenchSatCorr_ML_ions),2))" << "+(" << fitDATA->GetParameter(3) << "*pow(log10(BGO_EnergyG_QuenchSatCorr_ML_ions),3))" << ")\";" << endl;
    cout << endl;
    ...
    cout << endl;
    gwidth_data->Draw("p");
    gwidth_data->Fit("fitMC","r");
    cout << "TString sigma_MC = \"(" << fitMC->GetParameter(0) << "+(" << fitMC->GetParameter(1) << "*log10(BGO_EnergyG_SatCorr_ML_ions))" << "+(" << fitMC->GetParameter(2) << "*pow(log10(BGO_EnergyG_SatCorr_ML_ions),2))" << "+(" << fitMC->GetParameter(3) << "*pow(log10(BGO_EnergyG_SatCorr_ML_ions),3))" << ")\";" << endl;
    cout << endl;
