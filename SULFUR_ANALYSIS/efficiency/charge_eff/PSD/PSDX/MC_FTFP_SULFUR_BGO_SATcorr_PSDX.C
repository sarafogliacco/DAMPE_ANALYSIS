{
	...
	//TString meanX = "(((TMath::Sign(1.,PSD_ChargeX0)+1.)/2.*PSD_ChargeX0+(TMath::Sign(1.,PSD_ChargeX1)+1.)/2.*PSD_ChargeX1)/((TMath::Sign(1.,PSD_ChargeX0)+1.)/2.+(TMath::Sign(１.,PSD_ChargeX１)+１.)/２.))";
	//TString meanY = "(((TMath::Sign(１.,PSD_ChargeY０)+１.)/２.*PSD_ChargeY０+(TMath::Sign(１.,PSD_ChargeY１)+１.)/２.*PSD_ChargeY１)/((TMath::Sign(１.,PSD_ChargeY０)+１.)/２.+(TMath::Sign(１.,PSD_ChargeY１)+１.)/２.))";

	// -------------------------------------------------------------- 	HISTOGRAMS DEFINITION...
	
	TH1F *h01=new TH1F("h01", "100 GeV < E_{BGO} < 178 GeV",1200,10.0,20.0); h01->GetXaxis()->SetTitle("PSD X"); h01->GetYaxis()->SetTitle("normalized MC events"); h01->SetLineColor(kPink); h01->SetMarkerColor(kPink); h01->Sumw2();
	TH1F *h02=new TH1F("h02", "178 GeV < E_{BGO} < 316 GeV",1200,10.0,20.0); h02->GetXaxis()->SetTitle("PSD X"); h02->GetYaxis()->SetTitle("normalized MC events"); h02->SetLineColor(kPink); h02->SetMarkerColor(kPink); h02->Sumw2();
	TH1F *h03=new TH1F("h03", "316 GeV < E_{BGO} < 562 GeV",1200,10.0,20.0); h03->GetXaxis()->SetTitle("PSD X"); h03->GetYaxis()->SetTitle("normalized MC events"); h03->SetLineColor(kPink); h03->SetMarkerColor(kPink); h03->Sumw2();
	TH1F *h04=new TH1F("h04", "562 GeV < E_{BGO} < 1.0 TeV",1200,10.0,20.0); h04->GetXaxis()->SetTitle("PSD X"); h04->GetYaxis()->SetTitle("normalized MC events"); h04->SetLineColor(kPink); h04->SetMarkerColor(kPink); h04->Sumw2();
	TH1F *h05=new TH1F("h05", "1.0 TeV < E_{BGO} < 1.8 TeV", 600,10.0,20.0); h05->GetXaxis()->SetTitle("PSD X"); h05->GetYaxis()->SetTitle("normalized MC events"); h05->SetLineColor(kPink); h05->SetMarkerColor(kPink); h05->Sumw2();
	TH1F *h06=new TH1F("h06", "1.8 TeV < E_{BGO} < 3.2 TeV", 600,10.0,20.0); h06->GetXaxis()->SetTitle("PSD X"); h06->GetYaxis()->SetTitle("normalized MC events"); h06->SetLineColor(kPink); h06->SetMarkerColor(kPink); h06->Sumw2();
	TH1F *h07=new TH1F("h07", "3.2 TeV < E_{BGO} < 5.6 TeV", 600,10.0,20.0); h07->GetXaxis()->SetTitle("PSD X"); h07->GetYaxis()->SetTitle("normalized MC events"); h07->SetLineColor(kPink); h07->SetMarkerColor(kPink); h07->Sumw2();
	TH1F *h08=new TH1F("h08", "5.6 TeV < E_{BGO} < 10.0 TeV",600,10.0,20.0); h08->GetXaxis()->SetTitle("PSD X"); h08->GetYaxis()->SetTitle("normalized MC events"); h08->SetLineColor(kPink); h08->SetMarkerColor(kPink); h08->Sumw2();
	TH1F *h09=new TH1F("h09","10.0 TeV < E_{BGO} < 31.6 TeV",600,10.0,20.0); h09->GetXaxis()->SetTitle("PSD X"); h09->GetYaxis()->SetTitle("normalized MC events"); h09->SetLineColor(kPink); h09->SetMarkerColor(kPink); h09->Sumw2();
	TH1F *h10=new TH1F("h10","31.6 TeV < E_{BGO} < 100.0 TeV",600,10.0,22.0);h10->GetXaxis()->SetTitle("PSD X"); h10->GetYaxis()->SetTitle("normalized MC events"); h10->SetLineColor(kPink); h10->SetMarkerColor(kPink); h10->Sumw2();
	...
	// -------------- h01
	c0_1->cd(); for(int i=0; i<NUM_SET; i++){  skim[i]->Draw(avX+">>+h01",ctot*wSN[i]*bgo01,"");} cout<<"... h01 ..."<<endl;
	// -------------- h02
	c0_2->cd(); for(int i=0; i<NUM_SET; i++){  skim[i]->Draw(avX+">>+h02",ctot*wSN[i]*bgo02,"");} cout<<"... h02 ..."<<endl;
	// -------------- h03
	c0_3->cd(); for(int i=0; i<NUM_SET; i++){  skim[i]->Draw(avX+">>+h03",ctot*wSN[i]*bgo03,"");} cout<<"... h03 ..."<<endl;
	// -------------- h04
	c0_4->cd(); for(int i=0; i<NUM_SET; i++){  skim[i]->Draw(avX+">>+h04",ctot*wSN[i]*bgo04,"");} cout<<"... h04 ..."<<endl;
	// -------------- h05
	c0_5->cd(); for(int i=0; i<NUM_SET; i++){  skim[i]->Draw(avX+">>+h05",ctot*wSN[i]*bgo05,"");} cout<<"... h05 ..."<<endl;
	// -------------- h06
	c0_6->cd(); for(int i=0; i<NUM_SET; i++){  skim[i]->Draw(avX+">>+h06",ctot*wSN[i]*bgo06,"");} cout<<"... h06 ..."<<endl;
	// -------------- h07
	c0_7->cd(); for(int i=0; i<NUM_SET; i++){  skim[i]->Draw(avX+">>+h07",ctot*wSN[i]*bgo07,"");} cout<<"... h07 ..."<<endl;
	// -------------- h08
	c0_8->cd(); for(int i=0; i<NUM_SET; i++){  skim[i]->Draw(avX+">>+h08",ctot*wSN[i]*bgo08,"");} cout<<"... h08 ..."<<endl;
	// -------------- h09
	c0_9->cd(); for(int i=0; i<NUM_SET; i++){  skim[i]->Draw(avX+">>+h09",ctot*wSN[i]*bgo09,"");} cout<<"... h09 ..."<<endl;
	// -------------- h10
	c0_10->cd(); for(int i=0; i<NUM_SET; i++){ skim[i]->Draw(avX+">>+h10",ctot*wSN[i]*bgo10,"");} cout<<"... h10 ..."<<endl;
	
	// -------------------------------------------------------------- 	SAVE ON A FILE...
	TFile *f = TFile::Open("MC_FTFP_SULFUR_PSDXpathweighted_SATcorr_STKcut1200_PSDX.root", "RECREATE");
	f->cd();
