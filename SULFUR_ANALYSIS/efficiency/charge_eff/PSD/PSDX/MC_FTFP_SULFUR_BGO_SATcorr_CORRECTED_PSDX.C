{
	...
	TString avX_expr = "(((TMath::Sign(1.,PSD_ChargeX0)+1.)/2.*PSD_ChargeX0*PSD_psdX_pathlength[0]+(TMath::Sign(1.,PSD_ChargeX1)+1.)/2.*PSD_ChargeX1*PSD_psdX_pathlength[1])/((TMath::Sign(1.,PSD_ChargeX0)+1.)/2.*PSD_psdX_pathlength[0]+(TMath::Sign(1.,PSD_ChargeX1)+1.)/2.*PSD_psdX_pathlength[1]))";
...
auto MPV_MC = [](double E){
    if (E <= 0) return 0.0;
    double x = log10(E);
    return 15.084 + 1.63068*x  - 0.425641*x*x + 0.0386055*x*x*x;
};

auto sigma_MC = [](double E){
    if (E <= 0) return 1.0;
    double x = log10(E);
    return 0.251248 - 0.0375112*x + 0.00973476*x*x;
};
...
auto sigma_DATA = [](double E){
	...
	TH1F *h01=new TH1F("h01", "100 GeV < E_{BGO} < 178 GeV",1200,10.,20.); h01->GetXaxis()->SetTitle("PSD X"); h01->GetYaxis()->SetTitle("normalized MC events"); h01->SetLineColor(kViolet); h01->SetMarkerColor(kViolet); h01->Sumw2();
	TH1F *h02=new TH1F("h02", "178 GeV < E_{BGO} < 316 GeV",1200,10.,20.); h02->GetXaxis()->SetTitle("PSD X"); h02->GetYaxis()->SetTitle("normalized MC events"); h02->SetLineColor(kViolet); h02->SetMarkerColor(kViolet); h02->Sumw2();
	TH1F *h03=new TH1F("h03", "316 GeV < E_{BGO} < 562 GeV",1200,10.,20.); h03->GetXaxis()->SetTitle("PSD X"); h03->GetYaxis()->SetTitle("normalized MC events"); h03->SetLineColor(kViolet); h03->SetMarkerColor(kViolet); h03->Sumw2();
	TH1F *h04=new TH1F("h04", "562 GeV < E_{BGO} < 1.0 TeV",1200,10.,20.); h04->GetXaxis()->SetTitle("PSD X"); h04->GetYaxis()->SetTitle("normalized MC events"); h04->SetLineColor(kViolet); h04->SetMarkerColor(kViolet); h04->Sumw2();
	TH1F *h05=new TH1F("h05", "1.0 TeV < E_{BGO} < 1.8 TeV", 1200,10.,20.); h05->GetXaxis()->SetTitle("PSD X"); h05->GetYaxis()->SetTitle("normalized MC events"); h05->SetLineColor(kViolet); h05->SetMarkerColor(kViolet); h05->Sumw2();
	TH1F *h06=new TH1F("h06", "1.8 TeV < E_{BGO} < 3.2 TeV", 1200,10.,20.); h06->GetXaxis()->SetTitle("PSD X"); h06->GetYaxis()->SetTitle("normalized MC events"); h06->SetLineColor(kViolet); h06->SetMarkerColor(kViolet); h06->Sumw2();
	TH1F *h07=new TH1F("h07", "3.2 TeV < E_{BGO} < 5.6 TeV", 1000,10.,20.); h07->GetXaxis()->SetTitle("PSD X"); h07->GetYaxis()->SetTitle("normalized MC events"); h07->SetLineColor(kViolet); h07->SetMarkerColor(kViolet); h07->Sumw2();
	TH1F *h08=new TH1F("h08", "5.6 TeV < E_{BGO} < 10.0 TeV",1000,10.,20.); h08->GetXaxis()->SetTitle("PSD X"); h08->GetYaxis()->SetTitle("normalized MC events"); h08->SetLineColor(kViolet); h08->SetMarkerColor(kViolet); h08->Sumw2();
	TH1F *h09=new TH1F("h09","10.0 TeV < E_{BGO} < 31.6 TeV",1000,10.,20.); h09->GetXaxis()->SetTitle("PSD X"); h09->GetYaxis()->SetTitle("normalized MC events"); h09->SetLineColor(kViolet); h09->SetMarkerColor(kViolet); h09->Sumw2();
	TH1F *h10=new TH1F("h10","31.6 TeV < E_{BGO} < 100.0 TeV",500,10.,22.);h10->GetXaxis()->SetTitle("PSD X"); h10->GetYaxis()->SetTitle("normalized MC events"); h10->SetLineColor(kViolet); h10->SetMarkerColor(kViolet); h10->Sumw2();
...
for (int i = 0; i < NUM_SET; i++) {
    ...
    skim[i]->SetBranchAddress("PSD_psdX_pathlength", PSD_psdX_pathlength);
}
...
for (int i = 0; i < NUM_SET; i++) {
    ...
    for (Long64_t j = 0; j < n; j++) {
        ...
        // ================= PSD X medio =================
        double sX0 = (TMath::Sign(1., PSD_ChargeX0) + 1.) / 2.;
        double sX1 = (TMath::Sign(1., PSD_ChargeX1) + 1.) / 2.;

        double den =
            sX0 * PSD_psdX_pathlength[0] +
            sX1 * PSD_psdX_pathlength[1];

        if (den == 0) continue;

        double num =
            sX0 * PSD_ChargeX0 * PSD_psdX_pathlength[0] +
            sX1 * PSD_ChargeX1 * PSD_psdX_pathlength[1];
	...
	*/
	// -------------------------------------------------------------- 	SAVE ON A FILE...
	TFile *f = TFile::Open("MC_FTFP_SULFUR_SATcorr_STKcut1200_PSDcutXY_CORRECTED_PSDX.root", "RECREATE");
	f->cd();
