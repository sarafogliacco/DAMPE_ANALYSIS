...
void efficienza_MC_SULFUR()
{
    /////  HIST ////
    TFile aa("out_eff/eff_MC_FTFP_SULFUR_STK2nd_layer_8bin.root");
    aa.ls();
    TH1F* h1_Unb = (TH1F*) aa.Get("h1_Unb");
    TH1F* h1Nobs_And = (TH1F*) aa.Get("h1Nobs_And");
    TH1F* h1_HET_Eff = (TH1F*) aa.Get("h1_HET_Eff");
    ...
    std::vector<double> W(nbin_data+1, 0.0); 
    std::vector<double> DEN(nbin_data+1, 0.0); 
    std::vector<double> epsilon(nbin_data+1, 0.0); 
    ...
    for(int i=1; i<nbin_data+1; i++) {  // NEGLI ISTOGRAMMI IN INPUT h1Nobs_And & h1_Unb I BIN SONO 33 PER CUI IL CONTEGGIO CORRETTO VA DA i = 1 A i = 33 (*)
        epsilon[i] = h1_HET_Eff->GetBinContent(i);
        W[i] = h1Nobs_And->GetBinError(i);
        DEN[i] = h1_Unb->GetBinContent(i);
        nnum[i]=(DEN[i]/W[i])*(DEN[i]/W[i])*(h1Nobs_And->GetBinContent(i));
        nden[i]=(DEN[i]/W[i])*(DEN[i]/W[i])*(h1_Unb->GetBinContent(i));
        if (nden[i]==(1./0.)) { DEN[i] = 0;} 
    ...
    TEfficiency* pEff = new TEfficiency("eff"," ; ;STK cut Efficiency",32,2.,6.);
    bool bPassed;
    double x;
    ...
    TFile *f = TFile::Open("out_eff/eff_MC_SULFUR_STK2nd_layer_8bin.root", "RECREATE");
    f->cd();
